...
public:
	...
	virtual int GetSetDebugTraceCounter( int value, DebugTraceCounterBehavior_t behavior )OVERRIDE;
...
void CEngineTrace::GetBrushesInAABB( const Vector &vMins, const Vector &vMaxs, CBrushQuery &BrushQuery, int nContentsMask, int nCModelIndex )
{
	...
	if ( nContentsMask == CONTENTS_CURRENT_0/*CONTENTS_BRUSH_PAINT*/ && !host_state.worldbrush->m_pSurfaceBrushList )
	{
...
void CEngineTrace::ClipRayToCollideable( const Ray_t &ray, unsigned int fMask, ICollideable *pEntity, trace_t *pTrace )
{
	...
	if ( bIsStudioModel && !bTracedHitboxes && pTrace->DidHit() && (!bCustomPerformed || pTrace->surface.surfaceProps == 0) )
	{
		...
		pTrace->surface.surfaceProps = physprops->GetSurfaceIndex( pStudioHdr->pszSurfaceProp() );
	}
...
{
	fltx4 f4Min0 = LoadUnaligned3SIMD( &aabb0.m_vMinBounds );
	fltx4 f4Min1 = LoadUnaligned3SIMD( &aabb1.m_vMinBounds );
	fltx4 f4Max0 = LoadUnaligned3SIMD( &aabb0.m_vMaxBounds );
	fltx4 f4Max1 = LoadUnaligned3SIMD( &aabb1.m_vMaxBounds );
...
CON_COMMAND_F( occlusion_stats, "Occlusion statistics; [-jitter] [-reset]", FCVAR_NONE )
{
...
bool IsFullyOccluded_WithShadow( const AABB_t &aabb1, const AABB_t &aabb2, const Vector &vShadow, float flExtraMoveTolerance = 0.0f )
{
	...
	{
		Vector vAbsVect;
		VectorAbs( vCenter1 - vCenter2, vAbsVect );
		Vector vHullDist;
		VectorMax( vec3_origin, vAbsVect - ( vHullExtents1 + vHullExtents2 ), vHullDist ); // distance between hulls..
		  ...
		  && IsCastingShadow( aabb1 ) )
		{
			...
			{
				AABB_t aabbEx;
				VectorMin( aabb1.m_vMinBounds - Vector( flHorzMargin, flHorzMargin, 0 ), tr.vEndMin, aabbEx.m_vMinBounds );
				VectorMax( aabb1.m_vMaxBounds + Vector( flHorzMargin, flHorzMargin, flJumpMargin ), tr.vEndMax, aabbEx.m_vMaxBounds );
