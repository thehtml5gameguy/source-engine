...
CObjectTeleporter *CTFBotTacticalMonitor::FindNearbyTeleporter( CTFBot *actor )
{
	...
	{
		if ( objects[i]->ObjectType() != OBJ_TELEPORTER_ENTRANCE && objects[i]->ObjectType() != OBJ_TELEPORTER_EXIT)
			continue;

		CObjectTeleporter *pTele = static_cast<CObjectTeleporter *>( objects[i] );
		pTele->UpdateLastKnownArea();

		if ( pTele->ObjectType() != OBJ_TELEPORTER_ENTRANCE || !(pTele->GetState() == TELEPORTER_STATE_READY) || pTele->GetLastKnownArea() == nullptr || nearby.IsEmpty() )
			continue;
...
bool CTFBotTacticalMonitor::ShouldOpportunisticallyTeleport( CTFBot *actor ) const
{
	...
	{
		return ( actor->GetObjectOfType( OBJ_TELEPORTER_ENTRANCE ) != nullptr );
	}
	else
	{
