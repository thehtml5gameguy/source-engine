...
void CTFBotVision::UpdatePotentiallyVisibleNPCs()
{
	...
	for ( int i=0; i < IBaseObjectAutoList::AutoList().Count(); ++i )
	{
		...
		if ( pObj->GetType() == OBJ_SENTRYGUN || ( pObj->GetType() == OBJ_DISPENSER && pObj->ClassMatches( "obj_dispenser" ) ) || pObj->GetType() == OBJ_TELEPORTER_ENTRANCE || pObj->GetType() == OBJ_TELEPORTER_EXIT )
		{
