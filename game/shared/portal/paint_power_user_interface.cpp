		...
		else if( !pEnt->IsPlayer() )
		{
			...
			else
			{
				...
				if( FClassnameIs( pEnt, "phys_bone_follower" ) )
				{
					...
					if( pBoneOwner )
					{
						color32 render_color = pBoneOwner->GetRenderColor();
						info.m_PaintPowerType = MapColorToPower( Color( render_color.r, render_color.g, render_color.b ) );
					}
				}
				else //Use the render color of this entity
				{
					color32 render_color = pEnt->GetRenderColor();
					info.m_PaintPowerType = MapColorToPower( Color( render_color.r, render_color.g, render_color.b ) );
				}
